...
<header>
    <div id="language-flag-switcher">
        <?php if (Language::$Language == 'en') : ?>
            <a href="<?= Language::RouteUrl(Router::$CurrentControllerPath, 'hu'); ?>" onclick="return p3x.Language.Switch('hu', event );">
                <img alt="<?= Language::Get('layout', 'country-hu') ?>" class="language-flag" src="<?= Router::Url('images/language-flag-hungary.png') ?>"/>
            </a>
        <?php else : ?>
            <a href="<?= Language::RouteUrl(Router::$CurrentControllerPath, 'en'); ?>" onclick="return p3x.Language.Switch('en', event );">
                <img alt="<?= Language::Get('layout', 'country-us') ?>" class="language-flag" src="<?= Router::Url('images/language-flag-usa.png') ?>"/>
            </a>
        <?php endif ?>
    </div>
    ...
    <nav id="layout-top" class="navbar navbar-default navbar-fixed-top">
        <div class="container-fluid">
            <div class="navbar-header">
                <button id="navigation-menu-button" type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navigation" onclick="$(this).toggleClass('change');">
                    <span id="menu-bar-1" class="icon-bar"></span>
                    <span id="menu-bar-2" class="icon-bar"></span>
                    <span id="menu-bar-3" class="icon-bar"></span>
                </button>
                <a class="navbar-brand" p3x-ajax-href="<?= Language::RouteUrl($default_url); ?>" href="<?= Language::RouteUrl($default_url); ?>">
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <?php foreach ($orignal_themes as $theme) : ?>
                                <li id="theme-<?= $theme ?>" class="keep <?= $theme == $current_theme ? 'current-theme active' : '' ?>">
                                    <a href="javascript:lm.SwitchTheme('<?= $theme ?>');" style="text-transform: capitalize;">
                                        <?= $theme ?>
                                    </a>
                                </li>
                            <?php endforeach; ?>
                            ...
                            <?php foreach ($dark_themes as $theme) : ?>
                                <li id="theme-<?= $theme ?>" class="keep <?= $theme == $current_theme ? 'active' : '' ?>">
                                    <a href="javascript:lm.SwitchTheme('<?= $theme ?>');" style="text-transform: capitalize;">
                                        <?= $theme ?>
                                    </a>
                                </li>
...
<footer>
    ...
    <div class="p3x-sponsor fader">
        ...
        <a target="_blank" href="https://www.jetbrains.com/?from=patrikx3"><img src="https://cdn.corifeus.com/assets/svg/jetbrains-logo.svg" style="height: 50px; width: auto;"/></a>
        &nbsp;
    ...
    <div class="text-extended transition p3x-info fader">
        ...
        <span class="p3x-rs-status-divider">|</span>
        <span class="p3x-rs-status">
            <a data-toggle="modal" data-target="#layout-status-modal" p3x-ajax-href="<?= Router::Url('modal/status') ?>" href="<?= Router::Url('modal/status') ?>">
